...
@keyframes rotate {
 ...
 <!-- Search Bar -->
<div id="scrollsearchbar" class="mt-6 sm:mt-2 w-full max-w-4xl mx-auto bg-white py-4 px-2 sm:p-6 rounded-3xl shadow-lg">
...
>
  ...
  <!-- Dropdown container -->
  <ul id="suggestionsList" class="absolute z-50 bg-white w-full mt-1 rounded-xl border border-gray-300 max-h-60 overflow-y-auto hidden">
  </ul>
</div>
...
<div id="cards" class="-mt-2 lg:-mt-10 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-4  max-w-8xl mx-auto px-4  relative inset-0 z-10 ">
...
document.addEventListener('click', (e) => {
  if (!document.getElementById('searchInput').contains(e.target) && !document.getElementById('suggestionsList').contains(e.target)) {
    document.getElementById('suggestionsList').classList.add('hidden');
  }
});
